...
__global__ void recolorKernel(unsigned char *input, unsigned char *output,
  ...
  int x2 =
      min_val_x + (curand(&d_state[thread_id]) & (max_val_x - min_val_x + 1));
  int y2 =
      min_val_y + (curand(&d_state[thread_id]) & (max_val_y - min_val_y + 1));
...
int main(int argc, char **argv) {
  ...
  launchRecolorKernel(d_input, d_output, img->width, img->height, img->channels,
                      d_colorMaps, pixelDistance);

  // Copy result back to host
  CUDA_CHECK(
      cudaMemcpy(img->data, d_output, imageSize, cudaMemcpyDeviceToHost));
